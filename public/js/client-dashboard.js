...
async function loadServices() {
    try {
        ...
        if (services.length === 0) {
            container.innerHTML = '<div class="empty-state"><p>Nessun servizio disponibile al momento.</p></div>';
            return;
        }
        ...
        container.innerHTML = services.map(service => `
            <div class="service-item">
                ...
                <button onclick="openBookingModal('${service.id}')" class="btn btn-primary">Prenota</button>
            </div>
        `).join('');
    } catch (error) {
...
async function loadBookings() {
    try {
        ...
        if (bookings.length === 0) {
            container.innerHTML = '<div class="empty-state"><p>Non hai ancora prenotazioni.</p></div>';
            return;
        }
            ...
            <div class="booking-item">
                ...
                <p>
                    ...
                    <span class="status ${booking.paymentStatus}">
                        ${booking.paymentStatus === 'held_in_escrow' ? 'Trattenuto in Escrow' : 'Rilasciato al Fornitore'}
                    </span>
                </p>
                ${booking.photoProof ? `
