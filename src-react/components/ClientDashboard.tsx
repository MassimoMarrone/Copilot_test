...
const ClientDashboard: React.FC<ClientDashboardProps> = ({ googleMapsApiKey }) => {
  ...
  const handleSearch = (query: string, location?: { lat: number; lng: number; address: string }) => {
  ...
  const handleBooking = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        alert('Prenotazione confermata! Il pagamento è stato trattenuto in escrow.');
        closeBookingModal();
        loadBookings();
      } else {
        const data = await response.json();
  ...
  return (
    <div className="client-dashboard">
      ...
      <div className="dashboard-section">
        ...
        <SearchBar onSearch={handleSearch} googleMapsApiKey={googleMapsApiKey} />
          ...
          {bookings.length === 0 ? (
            ...
            bookings.map(booking => (
              <div key={booking.id} className="booking-card">
                ...
                <p><strong>Importo:</strong> <span className="price">€{booking.amount.toFixed(2)}</span></p>
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleBooking}>
              ...
              <div className="info-box">
                ℹ️ <strong>Pagamento in Escrow:</strong> Il pagamento sarà trattenuto in modo sicuro
                fino al completamento del servizio. Il fornitore riceverà il pagamento solo dopo aver
                caricato la prova fotografica del lavoro completato.
              </div>
